...
\begin{lem}
    Let $(\cC,\cE)$ be a geometric setup. If $X\in \cC$ is admissible,
    then the span $(\pt \xleftarrow{} X \xrightarrow{} \pt) \in \Span{\cC}_{\cE}$
    is dualizable and self dual, with evaluation and unit given by the spans
...
Now, given a six-functor formalism $\cD\colon \Span{\cC}_{\cE}\to \pPrl{\cA}$ that is symmetric monoidal, we can reduce computing the
trace of a pullback functor to computed the trace of the map in the
span category. More precisely:
...
\begin{rmk}
\todo{I have several issues with this entire remark}
