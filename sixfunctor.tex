...
\begin{defn}\label{defn: cohom traceable}
  Let $\Lambda = Y\xleftarrow{f} X \xrightarrow{g} Y$ be an endomorphism
  of some $Y\in \cC$ in $\Span{\cC}_{\cE,\cI,\cP}$. Say that $\Lambda$
  is \tdef{cohomologically traceable} if it satisfies the assumptions of 
  \cref{prop:internal-left-adjoints}. 
\end{defn}
...
\begin{prop}\label{prop: coincidence triangle}
    Let $\Lambda= \left( Y \xleftarrow{g} X \xrightarrow{f} Y\right)$ be
    a cohomologically traceable span with coincidence locus $X^{\Lambda}$. 
