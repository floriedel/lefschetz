...
\begin{prop}
  ...
  \begin{enumerate}[label=(\alph*)]
  \item The 1-morphism $\pt\leftarrow X\xrightarrow\id X$ has an internal right adjoint if $X$ is \'etale.
  \item The 1-morphism $X\xleftarrow{\id} X\to \pt$ has an internal right adjoint if $X$ is proper.
  \end{enumerate}
\end{prop}
\begin{proof}
  In the case of (a), the adjunction is witnessed by the 2-morphisms
  \[
    \begin{tikzpicture}[baseline=(current bounding box.center)]
      \node (X) at (-1.5,0){$\pt$};
...
\begin{prop}[The Lefschetz trace formula]
  ...
  Furthermore,
  \begin{enumerate}[label=(\alph*)]
  \item If $X$ is cohomologically proper, then $(X^f)_!(X^f)^*\mathbf 1\to\mathbf 1$ is induced by the projection $X^f\to\pt$.
  \item If $X$ is cohomologically \'etale, then $\mathbf 1\to (X^f)_!(X^f)^*\mathbf 1$ is induced by the projection $X^f\to\pt$.
  \end{enumerate}
\end{prop}
\begin{proof}
  $\ch{}f$ is the result of applying $\TrcL$ to the morphism
  \[
  ...
  \]
  in $\Endtrl (\pPrlf\cA )$.
  We can factor this diagram as the composition
  \[
    \begin{tikzcd}[column sep=huge]
      \cA\arrow[d,equal]
      \arrow[r,"X_*X^*"]
    ...
    \end{tikzcd}
  \]
  \Onote{write out diagram, finish writing proof.}
\end{proof}
