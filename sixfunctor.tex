...
\begin{defn}
  Suppose that $X\in\cC$ is suave and prim. The \tdef{Reidemeister trace} of a cohomologically \'etale endomorphism $f\curvearrowright X$, denoted $\mdef{\chr{}{f}}$, is the $\cA$-linear trace of the induced morphism
  \begin{equation}
...
\end{defn}
\begin{war}
  Usually, the term ``Reidemeister trace'' refers to a map $\mathbf 1\to (X^f)_!(X^f)^*\mathbf 1$. This is not what we are calling the Reidemeister trace. In order to emphasize the parallels with the previous subsection, we will instead refer to the classical invariant as a cocharacter, see below.
\end{war}
...
\end{defn}
\paragraph{$\infty$-groupoids}
Consider the six-functor formalism of parametrized spectra on the $\infty$-category of $\infty$-groupoids, induced via the Liu--Zheng construction by the symmetric monoidal functor
\[
  \begin{tikzcd}
    \spaces^\op\arrow r & \Prlst\\[-20pt]
    \anima\arrow[r,mapsto] & \Fun (\anima ,\Sp ),
  \end{tikzcd}
\]
which sends a map $f\colon B\to\anima$ to the pullback functor $f^*\colon\Fun (\anima ,\Sp )\to\Fun (B,\Sp )$. That is, we take $(\cC ,\cE ) = (\spaces ,\textrm{all maps})$ and $(\cI ,\cP ) = (\textrm{all maps}, \textrm{equivalences})$.

Fix a finitely-dominated $\infty$-groupoid $\anima$ and a self-map $f\curvearrowright X$ (which, like all maps between $\infty$-groupoids, is \'etale). In this case, we write $\mdef{L^f\anima } = \anima^f$. Informally, points in $L^f\anima$ are pairs $(x,\gamma )$, where $a$ is a point in $\anima$ and $\gamma\colon a\to\gamma (a)$. \cref{prop:reidemeister-trace} supplies a commutative triangle
  \begin{equation}
    \label{eq:homotopical-lefschetz-trace}
    \begin{tikzcd}
      & \SS\lbrack L^f\anima\rbrack\arrow[dr]\\
      \SS\arrow[rr,"\chr {}f"] \arrow[ru]&&\SS,
    \end{tikzcd}
  \end{equation}
  The Reidemeister cocharacter $\SS\to\SS\lbrack L^f\anima\rbrack$ is the invariant usually referred to as the ``Reidemeister trace'' of $f$.
