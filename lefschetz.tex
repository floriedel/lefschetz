...
The first item on our agenda is to reinterpret the right-hand side of the Lefschetz fixed point formula~\eqref{eq:lefschetz-formula} at the level of categories. For $h\colon B \to C$ some map of anima, we write 
$g^{\ast}\colon \SS^C\to \SS^B$ for the induced map on spherical cochains.
Recall that if $\anima$ is some compact anima, then by Spanier--Whitehead duality the spectrum $\SS^\anima$ is dualizable, with dual given by the
spherical chains $\SS[\anima]$.
We find it useful to take the following definition as our starting point:

\begin{defn}\label{defn: lefschetz trace}
  For a compact anima $\anima$, the \tdef{Lefschetz trace} of a self-map $g\colon \anima\to \anima$ is the map
  \[ \mdef{L(g)}\coloneq \tr{g^*\colon \SS^\anima\to \SS^\anima}{\Sp}\colon\SS\to\SS.\]
  If $\anima=\ani{\tset}$ is the underlying anima of a topological space $\tset$ and $g$ is induced by a self-map $f\colon\tset\to\tset$, then we will also refer to $L(f)\coloneq L(g)$ as the Lefschetz trace of $f$.
\end{defn}
...
for any self map of a compact anima $g\colon \anima \to \anima$ the induced map
of spectra $g^{\ast}\colon \SS^\anima\to \SS^\anima$ is identified with a map 
\[ g^{\ast}\colon (\Sp, \SS^\anima)\to (\Sp, \SS^\anima) \in \Endtrl(\Prlstf) \]
We write
\[
...
$\TrcL{(\Sp,\SS^\anima)}= \SS$ and moreover:
% \begin{prop}
%   For a compact anima $X$ and a self-map $g\colon X\to X$, there is a homotopy
   \begin{equation}
     \label{eq:trace-lax-comm}
     L(g)=\tr{g^*\colon \SS^\anima \to \SS^\anima}{\Sp} \simeq
     \TrcL\left(g^\ast\colon (\Sp,\SS^\anima)\to (\Sp,\SS^X)
     \right)\in \pi_0\SS.
   \end{equation}
% \end{prop}
% \begin{proof}
...
\begin{prop}
  Let $\tset$ be a compact Hausdorff space that is locally of singular shape, and let $f\colon\tset\to\tset$ be a self-map. Denote the underlying anima of $\tset$ by $\anima$ and the induced self-map on $\anima$ by $g\colon \anima\to \anima$.
...
\begin{cor}[Lefschetz fixed point theorem]
  Let $\tset$ and $f$ be as in the previous theorem. If the Lefschetz trace of $f$ is nonzero, then $f$ has a fixed point.
\end{cor}
\begin{proof}
